...
public final class CheckMain {
    ...
    public static String execute(final Class clazz, final CheckInfo checkInfo) {
        ...
        // 检测代码规范
        final List<HashMap<String, List<String>>> lists;
        // 代码注释间距检测
        ...
        // 方法注释缺少注释记录
        final HashMap<String, List<String>> sMethodUnAnnotateMap = lists.get(0);
        // 方法中参数未添加 final 记录
        final HashMap<String, List<String>> sParamUnFinalMap = lists.get(1);
        // 方法非 static 记录
        final HashMap<String, List<String>> sMethodUnStaticMap = lists.get(2);
        // 方法注释缺少 @param 记录
        final HashMap<String, List<String>> sMethodLackParamMap = lists.get(3);
        // 方法注释缺少 @return 记录
        final HashMap<String, List<String>> sMethodLackReturnMap = lists.get(4);
        // 方法存在 @param 但缺少注释记录
        final HashMap<String, List<String>> sMethodUnAnnotateParamMap = lists.get(5);
        // 方法存在 @return 但缺少注释记录
        final HashMap<String, List<String>> sMethodUnAnnotateReturnMap = lists.get(6);
        // 非 public 方法记录
        final HashMap<String, List<String>> sMethodUnPublicMap = lists.get(7);
        // 无修饰符变量 记录
        final HashMap<String, List<String>> sNoModifierVariableMap = lists.get(8);
        // 方法返回值 void 记录
        final HashMap<String, List<String>> sMethodReturnVoidMap = lists.get(9);
